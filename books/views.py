...
def book_upload(request):
    ...
    return render(request, "admin/book_upload.html")
...
def bookshelf(request):
    ...
    page_number = request.GET.get('page')
    books = paginator.get_page(page_number)
    return render(request, "user/bookshelf.html", {"books": books})


def read_book(request, pk):
    book = get_object_or_404(Book, pk=pk)
    return render(request, "user/read_book.html", {"book": book})
...
def update_credentials(request):
    ...
    return render(request, "admin/update_credentials.html", {"form": form})
